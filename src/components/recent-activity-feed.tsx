...
export default function RecentActivityFeed({
  ...
  const { data: activityData, isLoading: activityLoading } = useApi<any>(
    `/activities?${queryString}`
  )

  const activities = useMemo(() => {
    return (
      activityData?.results?.map((activity: any) => {
      ...
      <div className="space-y-6">
        {activities.map((activity: any, i: number) => (
          <div key={activity.id || i} className="flex gap-4">
            <div
              className={`w-2 h-2 rounded-full ${activity.color} shrink-0 mt-1.5`}
            ></div>
                  ...
                  <>
                    ...
                    {activity.riskScore !== undefined && (
                      <span className="text-xs">
                        {" "}(Score:{" "}
                        <span className="font-bold">{activity.riskScore}</span>,{" "}
                        <span className="font-bold uppercase text-[10px]">
                          {activity.riskInterpretation?.replace("_", " ")}
                        </span>
                        )
                      </span>
                    )}
      ...
      {!userId && (
        <div className="pt-3 mt-1 border-t border-border/40">
          <Link href="/activities" className="block w-full">
            <Button
              variant="ghost"
              className="w-full text-xs font-black uppercase tracking-widest text-muted-foreground hover:text-primary group h-10"
            >
              View All Activities
