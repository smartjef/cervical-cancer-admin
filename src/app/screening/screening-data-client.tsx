...
export default function ScreeningDataPage() {
  ...
  // Fetch Data
  const { data: summary, isLoading: summaryLoading } = useApi<any>(
    `/admin/dashboard/summary?${queryParams}`,
  );
  const { data: screeningsData, isLoading: screeningsLoading } = useApi<any>(
    `/screenings?${queryParams}`,
  );
  ...
  return (
    ...
    >
        ...
        {summaryLoading
          ? Array(4)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className="p-5 md:p-6 md:bg-card md:rounded-2xl border-border/50 animate-pulse flex flex-col gap-4"
                >
          ...
          : stats.map((stat, index) => (
              <div
                key={stat.title}
                className={`p-5 md:p-6 md:bg-card md:rounded-2xl transition-all border-border/50 ${index % 2 === 0 ? "border-r" : ""} ${index < 2 ? "border-b" : ""} md:border-none`}
              >
      ...
      <Card className="border-none mb-8 bg-card overflow-hidden">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-lg font-bold text-foreground">
            Screening Performance Trends
          </CardTitle>
          <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-sm bg-primary opacity-20" />{" "}
              Actual
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-0.5 bg-primary border-t-2 border-dashed" />{" "}
              Target
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-0.5 bg-emerald-500" /> Follow-up Rate %
            </div>
          </div>
        </CardHeader>
      ...
      <DataTable
        columns={[
          ...
          },
          {
            ...
            cell: (item: any) => (
              <Link
                href={`/users/clients/${item.client?.id}`}
          ...
          },
          {
            ...
            cell: (item: any) => (
              <div className="text-xs text-muted-foreground">
                {item.client?.ward || item.client?.subcounty || "N/A"}
              </div>
            ),
            sortable: false,
          },
            ...
            cell: (item: any) => (
              <Badge
                ...
                className={`
                                ...
                                ${
                                  item.scoringResult?.interpretation?.includes(
                                    "HIGH",
                                  )
                            ...
                            `}
              >
                {item.scoringResult?.interpretation
                  ?.replace("_RISK", "")
                  .replace("MODERATE", "MEDIUM") || "N/A"}
              </Badge>
            ),
            sortable: true,
          },
          {
            ...
            cell: (item: any) => (
              <div className="font-bold text-primary text-sm">
                {item.scoringResult?.aggregateScore || 0}
              </div>
            ),
            sortable: true,
          },
          {
            ...
            cell: (item: any) => (
              <div className="font-bold text-muted-foreground">
                {item.scoringResult?.clientAge || "N/A"}
              </div>
            ),
            sortable: true,
          },
