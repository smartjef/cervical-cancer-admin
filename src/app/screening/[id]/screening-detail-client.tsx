...
import { Loader2, Calendar, MapPin, User, Activity, AlertCircle, Phone, ArrowLeft, Eye } from "lucide-react"
...
function FollowUpsSection({ screeningId }: { screeningId: string }) {
    ...
    if (isLoading) return <div className="h-20 flex items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-primary" /></div>
            ...
            <CardContent>
                <div className="space-y-4">
                    {followUps.map((fu: any) => (
                        <div key={fu.id} className="flex items-center justify-between p-4 bg-muted/30 rounded-lg group hover:bg-muted/50 transition-colors">
                            <div className="space-y-1">
                                ...
                                <p className="text-sm font-bold">
                                    Status: <span className={fu.completedAt ? "text-emerald-600" : fu.canceledAt ? "text-rose-600" : "text-amber-600"}>
                                        {fu.completedAt ? "Completed" : fu.canceledAt ? "Canceled" : "Pending"}
                                    </span>
                                </p>
                            </div>
                            <Button variant="ghost" size="icon" asChild className="opacity-0 group-hover:opacity-100 transition-opacity h-8 w-8 hover:bg-primary/10 hover:text-primary">
            ...
            <CardContent>
                <div className="space-y-4">
                    {referrals.map((ref: any) => (
                        <div key={ref.id} className="flex items-center justify-between p-4 bg-muted/30 rounded-lg group hover:bg-muted/50 transition-colors">
                            <div className="space-y-1">
                                <div className="flex items-center gap-2">
                                    <Badge variant="outline" className="text-[10px] uppercase font-bold tracking-wider">
                                        Referral
                                ...
                                <p className="text-sm font-bold">
                                    Status: <span className={
                                        ref.status === 'COMPLETED' ? "text-emerald-600" :
                                            ref.status === 'REFUSED' ? "text-rose-600" :
                                                "text-amber-600"
                                    }>
                                        {ref.status}
                                    </span>
                            ...
                            <Button variant="ghost" size="icon" asChild className="opacity-0 group-hover:opacity-100 transition-opacity h-8 w-8 hover:bg-secondary/10 hover:text-secondary">
