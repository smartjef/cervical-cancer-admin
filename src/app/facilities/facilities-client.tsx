...
export default function FacilitiesClient() {
    ...
    const countyOptions = useMemo(() => {
        ...
        const mapped = results.map((c: any) => ({
            label: c.name,
            value: c.id // Using ID for hierarchy lookup
        }))
    ...
    const subcountyOptions = useMemo(() => {
        ...
        const mapped = results.map((s: any) => ({
            label: s.name,
            value: s.id // Using ID for hierarchy lookup
        }))
    ...
    const wardOptions = useMemo(() => {
        ...
        const mapped = results.map((w: any) => ({
            label: w.name,
            value: w.id // Wards can use Name as value since it's the leaf, but for consistency we can use ID
        }))
    ...
    }, [page, limit, search, sortBy, sortOrder, countyFilter, subcountyFilter, wardFilter, typeFilter, countyOptions, subcountyOptions, wardOptions])

    // Fetch Facilities
    const { data: facilitiesData, isLoading, refetch } = useApi<any>(`/health-facilities?${queryParams}`)
    ...
    return (
        <DashboardShell title="Health Facilities" subtitle="Manage registered health facilities">
            <DataTable
                columns={[
                    {
                        ...
                        cell: (item: any) => (
                            <div className="flex flex-col">
                                <Link href={`/facilities/${item.id}`} className="font-bold text-foreground hover:underline flex items-center gap-2">
                                    <Building2 className="h-3 w-3 text-muted-foreground" />
                                    {item.name}
                                </Link>
                                <span className="text-[10px] text-muted-foreground">{item.owner || "Unknown Owner"}</span>
                            </div>
                        ),
                        sortable: true
                    },
                    {
                        ...
                        cell: (item: any) => <div className="font-mono text-xs font-bold text-primary">{item.kmflCode}</div>,
                        sortable: true
                    },
                    {
                        header: "Type",
                    ...
                    },
                    {
                        ...
                        cell: (item: any) => (
                            <div className="flex flex-col">
                                ...
                                <span className="text-[10px] text-muted-foreground truncate max-w-[150px]">{item.email || '-'}</span>
                            </div>
                        ),
                        sortable: false
                    }
                        ...
                        <div className="flex items-center gap-3">
                            <Popover>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" className="h-11 px-4 gap-2 border-2 font-bold text-[10px] uppercase tracking-wider bg-background hover:bg-muted/50 transition-colors">
                                        ...
                                        Filter
                                        {(typeFilter !== "all" || countyFilter !== "all" || subcountyFilter !== "all" || wardFilter !== "all") && (
                                            <Badge variant="secondary" className="ml-1 h-5 min-w-[20px] px-1 font-black bg-primary text-primary-foreground">
                                                {[typeFilter, countyFilter, subcountyFilter, wardFilter].filter(f => f !== "all").length}
                                            </Badge>
                                        )}
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-80 p-5 space-y-5" align="end">
                                    ...
                                    <div className="space-y-4">
                                        <div className="space-y-2">
                                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Facility Type</Label>
                                            <Combobox
                                                options={typeOptions}
                                                value={typeFilter}
                            ...
                            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                                <DialogTrigger asChild>
                                    <Button className="h-11 px-6 gap-2 bg-primary hover:bg-primary/90 text-white font-black text-[10px] uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-primary/20">
