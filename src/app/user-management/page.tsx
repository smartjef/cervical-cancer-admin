...
export default function UserManagementPage() {
    ...
    const { data: chpData, isLoading: chpLoading } = useApi<any>("/chps")
    const { data: activityData, isLoading: activityLoading } = useApi<any>("/activities")

    const chpList = chpData?.data?.map((chp: any) => ({
        name: chp.name,
        role: "CHP",
        status: chp.status || "Active",
        location: chp.healthFacility?.name || "Kibera Sub-county",
        phone: chp.phone || "+254700000000",
        lastActive: chp.updatedAt ? dayjs(chp.updatedAt).fromNow() : "Recently",
        performance: chp.performance || "90%",
        color: "emerald"
    })) || users
    ...
    const recentActivities = activityData?.data?.map((activity: any) => ({
        ...
        color: activity.type === 'error' ? 'bg-red-500' : 'bg-emerald-500'
    })) || [
        ...
        <DashboardShell title="User Management" subtitle="Pearl Hospital">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500 font-medium">{chpData?.total || 6} total users across all roles</p>
                </div>
                        ...
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-4xl font-bold text-gray-800">
                                    {stat.title === "CHPs" ? (chpData?.total || stat.value) : stat.value}
                                </span>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                                            ...
                                            <div className="flex flex-col">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="flex items-center gap-1.5 ml-2">
                                                        <div className={`w-1.5 h-1.5 ${user.status === 'Active' ? 'bg-emerald-500' : 'bg-gray-400'}`}></div>
                                                        <span className={`text-[9px] font-bold ${user.status === 'Active' ? 'text-emerald-600' : 'text-gray-400'} uppercase tracking-wider`}>{user.status}</span>
                                                    </div>
                                                </div>
                ...
                <div className="space-y-4">
                    <h3 className="text-base font-bold text-gray-700">Recent Activity</h3>
