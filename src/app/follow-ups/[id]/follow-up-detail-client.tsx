...
export default function FollowUpDetailClient({ id }: FollowUpDetailClientProps) {
    ...
    return (
        <DashboardShell 
            ...
            subtitle={`Viewing details for follow-up #${followUp.id.slice(-6).toUpperCase()}`}
        >
            <div className="flex items-center gap-4 mb-6">
                    ...
                    <Card className="border-none bg-card sticky top-6">
                        <CardHeader className="pb-2">
                            <div className="flex justify-between items-start">
                                <div className={`h-12 w-12 rounded-full flex items-center justify-center ${followUp.priority === 'HIGH' ? 'bg-rose-500/10 text-rose-600' : followUp.priority === 'MEDIUM' ? 'bg-amber-500/10 text-amber-600' : 'bg-sky-500/10 text-sky-600'}`}>
                                    <Activity className="h-6 w-6" />
                                </div>
                                <div className="flex flex-col items-end gap-1">
                                    <Badge variant="outline" className={`font-bold border-none px-2.5 py-0.5 rounded-full text-[10px] uppercase tracking-wider ${followUp.priority === 'HIGH' ? 'bg-rose-500/10 text-rose-600' : followUp.priority === 'MEDIUM' ? 'bg-amber-500/10 text-amber-600' : 'bg-sky-500/10 text-sky-600'}`}>
                                        {followUp.priority} Priority
                                    </Badge>
                                    <Badge variant="outline" className={`font-bold border-none px-2.5 py-0.5 rounded-full text-[10px] uppercase tracking-wider ${isCompleted ? 'bg-emerald-500/10 text-emerald-600' : isOverdue ? 'bg-rose-500/10 text-rose-600' : isCancelled ? 'bg-slate-500/10 text-slate-600' : 'bg-amber-500/10 text-amber-600'}`}>
                                ...
                                <div className="flex items-center gap-3 text-sm px-2">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <span className={`font-medium ${isOverdue ? 'text-rose-600' : ''}`}>
                                            {dayjs(followUp.dueDate).format("MMM D, YYYY")}
                                        </span>
                            ...
                            <CardContent>
                                <div className="p-4 bg-muted/30 rounded-lg flex items-center justify-between">
                                    <div>
                                        ...
                                        <Badge variant="outline" className={`mt-2 font-bold border-none px-2 py-0.5 rounded-full text-[9px] uppercase tracking-wider ${followUp.triggerScreening?.scoringResult?.interpretation?.includes('HIGH') ? 'bg-rose-500/10 text-rose-600' : 'bg-emerald-500/10 text-emerald-600'}`}>
