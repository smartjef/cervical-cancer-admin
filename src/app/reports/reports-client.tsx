...
export default function ReportsPage() {
  ...
  const { data: dashboardData, isLoading: isDashboardLoading } = useApi<any>(
    ...
    try {
      ...
      switch (selectedType) {
        ...
        default:
          toast({
            title: "Coming Soon",
            description: "This report type is currently under development.",
            variant: "default",
          });
      ...
      const finalData = finalDataRaw.map((item: any) => {
        ...
        if (selectedType === "compliance_quality") {
          ...
          mapped.appointmentTime = item.appointmentTime
        ...
        if (selectedType === "screening_summary") {
          ...
          mapped.clientName = item.client
            ...
            : "N/A";
          mapped.providerName = item.provider
      ...
      if (format === "csv" || format === "excel") {
        ...
        const addHeader = (doc: jsPDF) => {
          ...
          doc.setFont("helvetica", "normal");
          doc.text(
            `DATE GENERATED: ${dayjs().format("YYYY-MM-DD HH:mm")}`,
            150,
            15,
          );
        ...
        autoTable(doc, {
          ...
          didDrawPage: (data) => {
            ...
            doc.text(
              `© ${dayjs().format("YYYY")} SCREEN-IT - CONFIDENTIAL MEDICAL DATA`,
              pageSize.width / 2 - 40,
              pageHeight - 10,
            );
          },
      ...
      toast({
        title: "Success",
        description: `${filename}.${format === "pdf" ? "pdf" : "csv"} has been generated.`,
        variant: "success",
      });
    } catch (error) {
    ...
    return [
      {
        ...
        icon: Activity,
        description: "All-time screenings",
  ...
  return (
    <DashboardShell
      title="Reports"
      subtitle="SCREEN-IT Comprehensive Reporting System"
    >
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-foreground mb-1 tracking-tighter uppercase">
            Analytics & Reports
          </h2>
          <p className="text-sm text-muted-foreground font-bold uppercase tracking-wider opacity-70">
            Deep insights and data management for Pearl Hospital.
          </p>
        </div>
      </div>
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-8">
            <div>
              ...
              <CardDescription className="text-[10px] uppercase font-bold tracking-widest">
                Monthly screening activity overview
              </CardDescription>
            </div>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    exportToCSV(
                      data,
                      `clients-db-${dayjs().format("YYYY-MM-DD")}`,
                      [
                    ...
                    toast({
                      title: "Success",
                      description: "Clients database exported.",
                      variant: "success",
                    });
                  } catch (e) {
                    toast({
