...
export default function ReferralsClient() {
  ...
  // Fetch Referrals
  const { data: referralsData, isLoading } = useApi<any>(
    `/referrals?${queryParams}`,
  );
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const res = await apiRequest(`/referrals?${exportParams.toString()}`);
  ...
  return (
    <DashboardShell
      ...
      subtitle="Manage and track facility-based referrals"
    >
      <DataTable
        columns={[
          {
            ...
            cell: (item: any) => (
              ...
              >
                ...
                <span className="truncate">
                  {item.screening?.client?.firstName}{" "}
                  {item.screening?.client?.lastName}
                </span>
              </Link>
            ),
            sortable: false,
            ...
            cell: (item: any) => (
              <div className="flex flex-col max-w-[200px]">
                {item.healthFacilityId ? (
                  <Link
                    href={`/facilities/${item.healthFacilityId}`}
                    className="font-bold text-foreground truncate hover:underline flex items-center gap-1.5"
                  >
                ...
                <span className="text-[10px] text-muted-foreground truncate ml-4.5">
                  {item.healthFacility?.subcounty},{" "}
                  {item.healthFacility?.county}
                </span>
              </div>
            ),
            sortable: false,
            ...
            cell: (item: any) => (
              <div className="flex flex-col">
                <span className="text-xs font-bold text-foreground">
                  {dayjs(item.appointmentTime).format("MMM D, YYYY")}
                </span>
                <span className="text-[10px] text-muted-foreground">
            ...
            cell: (item: any) => (
              <div className="flex flex-col gap-1">
                {item.transportNeeded && (
                  <Badge
                    variant="outline"
                    className="bg-sky-500/10 text-sky-600 border-none text-[9px] w-fit"
                  >
                    Transport
                  </Badge>
                )}
                {item.financialSupport && (
                  <Badge
                    variant="outline"
                    className="bg-purple-500/10 text-purple-600 border-none text-[9px] w-fit"
                  >
                    Financial
                  </Badge>
                )}
